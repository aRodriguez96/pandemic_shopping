...
public class customerClass implements Runnable {
    ...
    public void run() {
        outsideLine();
        while (this.isAllowedInside.get() != true) {} //BW while not allowed inside
        enterStore();
        ...
        getOnCheckoutLine();
        if (this.isElderly.get() == true) {       //If the customer is elderly, it looks ahead of the line to see who can yield to them. The yielding stops if the customer reaches someone in line with 0 yields left
            ...
            if (checkoutTest.length > 0) {
                for (int i = 1; i < checkoutTest.length; i++) {
                    if (checkoutTest[i].isElderly.get() == true) {} else {
                        if (checkoutTest[i].yieldsLeft.get() > 0) {
                            checkoutTest[i].thread.yield();                //if customer ahead in line can yield, they yield to the elderly customer
        ...
        msg("Customer-" + this.ID + " leaves the store and goes to their car");     //goes to their car to sleep
        storeClass.parkingLotLine[Integer.parseInt(this.ID) - 1] = this;
        try {
            Thread.sleep(1000000000);    //sleep for a loooooooooooooooooong time
        } catch (InterruptedException e) {
